...
if [ "$#" -eq 1 ]; then
	...
	[lL] | [lL][uU][iI][gG][iI])
		PERSONAGEM=luigi
		PERSONAGEM_TITLE=LUIGI
		PERSONAGEM_CHAR=L
		;;

	*) 
		PERSONAGEM=mario
		PERSONAGEM_TITLE=MARIO
		PERSONAGEM_CHAR=M
		;;
...
function Splash {
	...
	_canvas[13]="${_edgeScreen}${_spaceIni}                             █░░░░${PERSONAGEM_CHAR}███                                          ${_spaceFim}${_edgeScreen}"
...
function DrawJogador {
	...
	for (( _l=_tmpIni; _l<=_tmpJogHeight; _l++ )); do
		...
		_tmp="_${PERSONAGEM}${_jogSide}${_jogSprite}[$_l]"
...
_scoreTitle0="     ${PERSONAGEM_TITLE}                                   WORLD               TIME           "
